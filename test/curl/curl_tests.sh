                    ...
                    if grep -f ./test/curl/post_expected.txt actual.txt; then
                        ...
                        if grep -f ./test/curl/post_no_auth.txt actual.txt; then
                            ...
                            if grep -f ./test/curl/put_no_auth.txt actual.txt; then
                                #Good Put
                                curl --silent -X PUT -H 'Content-Type: application/json' -H 'Authorization: cs4783ftw!' -d '{"address":"463 UTSA","city":"San Antonio","state":"Tx","zip":"78228","id":3}' https://10.100.201.3:12030/properties/id --insecure > actual.txt
